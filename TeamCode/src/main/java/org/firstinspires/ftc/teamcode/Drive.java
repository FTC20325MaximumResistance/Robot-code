...
public class Drive extends OpMode {
    ...
    public void loop() {
        if(!blank) {
            arm = new CraneArm(this, (DcMotor) r.crane_lift);
            spinner = new spinner(this, (DcMotor) r.spin);
            blank = true;
        }
        {
        ...
        // Max crane pos = 6276
        posi = String.valueOf(r.crane_lift.getCurrentPosition());
        //r.crane_lift.setPower(gamepad2.right_stick_y * 0.9);
        //r.spin.setPower(gamepad2.left_stick_x * 0.9);
        telemetry.addData("Crane position", posi);
        r.scoop.setPower(gamepad2.right_bumper ? 0.9 : 0);
        r.scoop.setPower(gamepad2.left_bumper ? -0.9 : 0);
        if (gamepad2.dpad_up){
            arm.get_dir(3);
            arm.start();
        }
